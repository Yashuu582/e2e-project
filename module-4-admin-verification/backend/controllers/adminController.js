...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    const pendingHostels = await Hostel.countDocuments({ isVerified: false });

    res.json({
      success: true,
      stats: { totalUsers, verifiedUsers, totalHostels, verifiedHostels, pendingHostels }
    });
  } catch (error) {
...
exports.getPendingHostels = async (req, res) => {
  try {
    const hostels = await Hostel.find({ isVerified: false })
